...
-- Single-row insert-or-skip for attendance_records. uq_attendance_student_event is
-- a PARTIAL unique index (WHERE event_id IS NOT NULL) — PostgREST's .upsert() has
-- no way to express that predicate in its ON CONFLICT target, so it can't safely
-- suppress conflicts against this index (PostgREST issue #2123 — upserting against
-- partial/expression indexes is still an open, unimplemented request). This
-- function repeats the exact predicate so Postgres can always resolve the arbiter.
-- Not SECURITY DEFINER: runs with the caller's own privileges, so the existing
-- "Council manages attendance" RLS policy gates every insert exactly as a direct
-- .insert() would.
...
-- Batch version for the backfill's absent-marking pass (backfillEventStatuses in
-- src/lib/attendance.ts). A single multi-row INSERT with ON CONFLICT DO NOTHING
-- resolves each row independently, so — unlike the JS-side binary-split-retry it
-- replaces — one colliding row (a concurrent scan/manual entry) never requires a
-- retry and never raises 23505. Never updates an existing row, so a real
